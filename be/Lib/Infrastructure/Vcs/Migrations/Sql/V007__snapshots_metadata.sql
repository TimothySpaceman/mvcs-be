...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260611185108_snapshots_metadata') THEN
    CREATE TABLE snapshot_metadata (
        "CommitId" character varying(32) NOT NULL,
        "ProjectId" uuid NOT NULL,
        "Data" jsonb NOT NULL,
        "SubmittedAt" timestamp with time zone NOT NULL,
        CONSTRAINT "PK_snapshot_metadata" PRIMARY KEY ("CommitId", "ProjectId")
    );
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260611185108_snapshots_metadata') THEN
    CREATE INDEX "IX_snapshot_metadata_Data" ON snapshot_metadata USING gin ("Data");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260611185108_snapshots_metadata') THEN
    CREATE INDEX "IX_snapshot_metadata_ProjectId" ON snapshot_metadata ("ProjectId");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260611185108_snapshots_metadata') THEN
    INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20260611185108_snapshots_metadata', '10.0.8');
    END IF;
END $EF$;
COMMIT;
